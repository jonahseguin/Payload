 ...
 * @ 5:50 PM
 */
public interface Cache<T extends Profile> {

    /**
     * Get a profile by a player object
     ...
     * by this method, and it created a blank profile for the player based on their username and uuid.
     ...
     * Note that this method effectively calls {@code return this.getProfile(player.getUniqueId()); }
    ...
    /**
     * Returns a Profile for the given player; only attempting to get from the local cache (no database calls)
     *
     * @param player Player
     ...
     *
     * @param uniqueId The player's {@link UUID#toString()}
     * @return The profile, or none if it does not exist anywhere (temporary joining cache -> local -> redis -> mongo)
     */
    T getProfile(String uniqueId);

    /**
     * Attempts to find a profile using the player's username as the query criteria.
