    ...
    public void execute(CmdArgs args) {
        ...
        if (cache.getPayloadDatabase() != null) {
            if (cache.requireMongoDb()) {
                args.msg("&7MongoDB Status: {0} (Init: {1})", (cache.getPayloadDatabase().getState().isMongoConnected() ? "&aConnected" : "&cDisconnected"), (cache.getPayloadDatabase().getState().isMongoInitConnect() ? "&aYes" : "&cNo"));
            }
            if (cache.requireRedis()) {
                args.msg("&7Redis Status: {0} (Init: {1})", (cache.getPayloadDatabase().getState().isRedisConnected() ? "&aConnected" : "&cDisconnected"), (cache.getPayloadDatabase().getState().isRedisInitConnect() ? "&aYes" : "&cNo"));
            }
        }
    }
