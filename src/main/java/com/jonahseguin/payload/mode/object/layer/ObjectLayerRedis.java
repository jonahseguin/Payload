...
public class ObjectLayerRedis<X extends PayloadObject> extends ObjectCacheLayer<X> {
    ...
    public X get(String key) {
        try (Jedis jedis = this.cache.getPayloadDatabase().getResource()) {
            String json = jedis.hget(this.getCache().getServerSpecificName(), key);
            if (json == null) {
                return null;
            }
    ...
    public boolean save(X payload) {
        ...
        try (Jedis jedis = this.cache.getPayloadDatabase().getResource()) {
            jedis.hset(this.getCache().getServerSpecificName(), payload.getIdentifier(), json);
            return true;
        } catch (Exception expected) {
    ...
    public boolean has(String key) {
        try (Jedis jedis = this.cache.getPayloadDatabase().getResource()) {
            return jedis.hexists(this.getCache().getServerSpecificName(), key);
        } catch (Exception expected) {
    ...
    public void remove(String key) {
        try (Jedis jedis = this.cache.getPayloadDatabase().getResource()) {
            jedis.hdel(this.getCache().getServerSpecificName(), key);
        } catch (Exception expected) {
    ...
    public long size() {
        try (Jedis jedis = this.cache.getPayloadDatabase().getResource()) {
            return jedis.hlen(this.cache.getServerSpecificName());
        } catch (Exception ex) {
